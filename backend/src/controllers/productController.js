...
const addProduct = async (req, res) => {
  ...
  try {
    ...
    // Ensure absolute URL is saved to avoid changing multiple frontend files
...
const editProduct = async (req, res) => {
  ...
  try {
    ...
    if (updates.storeId) updates.storeId = parseInt(updates.storeId);
...
const sortProducts = async (req, res) => {
  ...
  catch (error) { return res.status(500).json({ message: "Error updating product", error: error.message }); }
};
...
const getProductDetails = async (req, res) => {
  try {
    ...
    const product = await prisma.product.findUnique({
      where: { id: parseInt(productId) },
      include: {
        reviews: { orderBy: { createdAt: 'desc' } } // include reviews of the product 
      }
    })
